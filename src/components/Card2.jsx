  ...
  return (
    <div className="w-full max-w-md sm:max-w-lg md:max-w-xl bg-white rounded-xl shadow-lg p-4 flex flex-col sm:flex-row items-center gap-4 hover:shadow-2xl transition-shadow duration-300">
      ...
      <div className="flex-1 flex flex-col justify-between h-full w-full">
        <div>
          <h2 className="text-lg sm:text-base md:text-lg font-bold text-gray-800">{name}</h2>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mt-4 gap-2 sm:gap-0">
          ...
          {/* Delete Icon */}
          <AiFillDelete
  className="text-red-500 w-6 h-6 sm:w-5 sm:h-5 cursor-pointer hover:text-red-600 hover:scale-110 transition-transform mt-2 sm:mt-0"
  onClick={() => {
    dispatch(RemoveItem(id)); // remove item by id
  }}
/>
        </div>
      </div>
